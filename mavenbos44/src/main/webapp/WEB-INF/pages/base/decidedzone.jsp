...
<!-- 引入自定义js -->
<script
	src="${pageContext.request.contextPath }/js/jquery-extends-customization.js"
	type="text/javascript"></script>
<script type="text/javascript">
	function doAdd(){
	...
	function doAssociations(){
		...
		if(row==null){
			//提示用户
			$.messager.alert("提示","请选择一个定区","info");
			return ;
		}
		...
		//填充关联窗口的数据（两部分数据）
		//没有关联任何定区的客户列表
		$.post("${pageContext.request.contextPath}/decidedZone_listCustomerListNoDecidedZoneId.action",function(data){
			//data:json数组：data=[{},{},{}]
			$(data).each(function(){
				//option
				var option=$("<option value='"+this.id+"'>"+this.name+"("+this.telephone+")</option>");
				//添加到选项框中
		...
		//查询关联某个定区的客户列表(传入定区编号)
		$.post("${pageContext.request.contextPath}/decidedZone_listCustomerListHasDecidedZoneId.action",{"id":row.id},function(data){
			//data:json数组：data=[{},{},{}]
			$(data).each(function(){
				//option
				var option=$("<option value='"+this.id+"'>"+this.name+"("+this.telephone+")</option>");
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="decidedZoneForm" action="${pageContext.request.contextPath }/decidedZone_add.action" method="post">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"  
    							...
    							,url:'${pageContext.request.contextPath }/staff_listNoDel.action'" />  
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							...
							data-options="url:'${pageContext.request.contextPath }/subarea_listNoDecidedZoneId.action',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
			...
			<form id="queryForm">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>是否关联分区</td>
						<td>
						<select name="hasSubarea" class="easyui-combobox">
