...
<script type="text/javascript">
	...
	function doExport(){
		...
		//同步，不能异步；携带业务条件请求
		//动态给表单添加属性
		$("#queryForm").attr("action","${pageContext.request.contextPath}/subarea_exportData.action");
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			url : "${pageContext.request.contextPath}/subarea_listPage.action",
			idField : 'id',
			columns : columns,
			onDblClickRow : doDblClickRow
			//发送额外的业务参数
			//,queryParams:{"name":"Rose","age":18}
		...
		$("#btn").click(function(){
			//alert("执行查询...");
			//目标：先将参数转换为json对象，然后调用datagrid的load方法，将其缓存到datagrid中（queryParams）
			//1.获取表单的json对象
			//方法两种：
			//1)手工拼接
			/* var paramJson;
			...
			if(addresskey!=""){
				paramJson.addresskey=addresskey;
			} */
			
			//2)自动转换:表单序列化
			//jq:内置表单转换的工具方法
			//$("form").serialize()序列化为http参数：name=rose&age=18&.....
			//$("form").serializeArray()序列化为json数组对象！
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="分区添加修改" id="addSubareaWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="subareaForm" action="${pageContext.request.contextPath }/subarea_add.action" method="post">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="region.id"  
    							...
    							url:'${pageContext.request.contextPath }/region_listAjax.action'" />  
						</td>
					</tr>
					<tr>
						<td>关键字</td>
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="queryForm">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>省</td>
						<td><input type="text" name="region.province" class="easyui-validatebox" /></td>
					</tr>
					<tr>
						<td>市</td>
						<td><input type="text" name="region.city" class="easyui-validatebox" /></td>
					</tr>
					<tr>
						<td>区（县）</td>
						<td><input type="text" name="region.district" class="easyui-validatebox"/></td>
					</tr>
					<tr>
						<td>定区编码</td>
						<td><input type="text" name="decidedZone.id" class="easyui-validatebox" /></td>
					</tr>
					<tr>
						<td>关键字</td>
